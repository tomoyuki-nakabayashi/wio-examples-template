//! 10-1 ストップウォッチをつくる のサンプルコードです。
//! Wio Terminalをストップウォッチとして使えます。
//!
//! ### 実行方法
//! ```sh
//! $ cargo hf2 --example 10-1-stop_watch --features app --release
//! ```
...
fn main() -> ! {
    ...
    // パニック・ハンドラ用にUARTを初期化する (ストップウォッチの機能としては不使用)
    ...
    // デューティ比 (ON/OFFの比率) を0.5に
    // 音色変えたい場合は比率を変えると良い
    let max_duty = buzzer.get_max_duty();
    buzzer.set_duty(Channel::_4, max_duty / 2);
    buzzer.disable(Channel::_4);

    // 時間を計測するためにタイマーを初期化する
    // 正確に時間を計測するために、XOSC32K (外部32.768[kHz]水晶発振器) を基準にしたクロックを使う必要がある。
    ...
    tc3.start(62500.us()); // 62.5[ms] = 1/16[s] 周期のカウンタとしてTC3の動作を開始
    ...
    loop {
        match state {
            ...
            State::Running => {
                ...
                if button_stop.is_low().unwrap() {
                    ...
                    beep_buzzar(&mut buzzer, &mut delay, 880.hz(), 50u16);
                    delay.delay_ms(50u16);
                    beep_buzzar(&mut buzzer, &mut delay, 880.hz(), 100u16);
        ...
        writeln!(&mut textbuffer, "{:.2}", elapsed_s).unwrap();
        let length = textbuffer.len() - 1; // textbufferの末尾には終端用のヌル文字が入っていて、len()はそれを含む長さを返すので-1しておく
        let left = SCREEN_WIDTH - (length as i32) * FONT_WIDTH; // 右詰描画用に左の座標計算
        egtext!(
